<template>
  <svg width="100%" viewBox="0 0 583 61" fill="none" xmlns="http://www.w3.org/2000/svg">
    <!-- Profile shape -->
    <path opacity="0.7" d="M567 50V6C467.239 40.9744 198.345 47.5556 16 50H567Z" :fill="gradientUrl"/>
    ...
    <template v-if="isLatched">
      ...
      <circle cx="0" cy="0" r="3.0" fill="#F9AC20">
        <animateMotion dur="1.75s" repeatCount="indefinite" keyPoints="0;1" keyTimes="0;1" calcMode="spline" keySplines="0.4 0 1 1">
          <mpath :href="`#exp-curve-${uid}`"/>
        </animateMotion>
      </circle>
      <circle cx="0" cy="0" r="3.0" fill="#F9AC20">
        <animateMotion dur="1.75s" repeatCount="indefinite" keyPoints="1;0" keyTimes="0;1" calcMode="spline" keySplines="0.4 0 1 1">
          <mpath :href="`#exp-curve-${uid}`"/>
        </animateMotion>
      </circle>
    </template>
    ...
    <defs>
      <!-- Animation path -->
      <path :id="`exp-curve-${uid}`" d="M16 50 C198.345 47.5556 467.239 40.9744 567 6"/>
      
      <!-- Gradient -->
      <linearGradient :id="`exp-gradient-press-${uid}`" x1="16" y1="28" x2="567" y2="28" gradientUnits="userSpaceOnUse">
...
const gradientUrl = computed(() => `url(#exp-gradient-press-${uid})`)
</script>
